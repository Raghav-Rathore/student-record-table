...
export default class App extends Component {
  ...
  render() {
    ...
    return <>
              <h1 style={{color :"red",textAlign:"center",fontStyle:"italic"}} > Created By RR</h1>
      ...
      <div className="container mt-4">
        ...
        <form onSubmit={this.add}>
          ...
          <div className="row mt-3">
            ...
            <div className="col-md-8 text-center" >
              <button className="btn btn-primary btn-lg" type="submit"  disabled={this.state.duplicateRoll}> Register </button>
            </div>

          </div>
        </form>
        {this.course.map(ob=><button onClick={()=>this.setState({selectCourse:ob})} style={{marginLeft:"5px"}}  className="btn btn-primary">{ob}
              : {this.state.students.reduce((x,obj)=> obj.course==ob ?x+1:x,0)}        </button>)}
        <button className="btn btn-success" onClick={()=>this.setState({selectCourse:undefined})}> Total</button>
        <table className="table table-stripped mt-3">
          ...
          <tbody>
            {this.state.students.filter(ob=>this.state.selectCourse==undefined||ob.course==this.state.selectCourse).map(ob =>
              <tr>
                <td>{ob.roll}</td>
                <td>{ob.name}</td>
                <td>{ob.age}</td>
